  ...
  module ETI
    class Table
      class Products < self
        ...
        elements(:new_empty_product, :row, css: '.js-upload-field.new')
        elements(:products, :row, xpath: '//tr[contains(@class,"js-upload-field") and not(contains(@class, "new"))]')
        elements(:names, :cell, css: '.js-eti-name')

        def add_product(params = {})
          add_new_product
          wait_until { new_empty_product_elements[0] }
          self.name = params[:name] if params.key? :name
        end

        def name=(text, product = nil)
          browser
            .action
            .move_to(product.nil? ? names_elements[0].element : product)
            .click
        ...
        def where(params)
          if params.key? :name
            names_elements.select { |n| n.text == params[:name] }
          elsif params.key? :id
            products_elements.select { |p| p.attribute('data-id') == params[:id] }
          else
            []
          end
        end
